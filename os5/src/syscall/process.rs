...
use crate::mm::{translated_refmut, translated_str, get_phyaddr};
use crate::task::{
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        let task = current_task().unwrap();
        let process_block = task.spawn(data);
        let pid = process_block.pid.0 as isize;
        add_task(process_block);
        pid
    } else {
        -1
    }
...
pub fn update_syscall_times(syscall_id: usize) {
    _update_syscall_times(syscall_id);
}